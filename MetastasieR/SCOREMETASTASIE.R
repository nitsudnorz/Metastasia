...
###to dataframe
equations = (matrix(unlist(equation_list), nrow =length(l), byrow=TRUE))
...
writelineeq <- function(eq,t)
  ...
  lineq <- paste(deparse1(doodoodoodooExpr))
  print((gsub("x", t , lineq )))
}

writelineeq(d[1,2],2)

asdfdsf <- (-((x - 55 - 2)^2 + 3 * (x - 55) + 140))

expression(-((x - 55 - 2)^2 + 3 * (x - 55) + 140))
as.expression(asdfdsf)
